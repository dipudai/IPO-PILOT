...
func generateKey(name string) string {
	hash := md5.Sum([]byte(name + "IPO_Pilot"))
...
func activateLicense() bool {
	fmt.Println(cyan("\n=== App Activation ==="))
	fmt.Println("Please enter your activation password to activate IPO Pilot")
	fmt.Println("Contact the developer for activation password.")
	fmt.Println("Developer: dallefx")
	fmt.Println("Email: sahmahesh2077@gmail.com")
	fmt.Print("Activation Password: ")
	...
	if password == "" {
		fmt.Println(red("Activation password cannot be empty."))
		return false
	}
	...
	if password != correctPassword {
		fmt.Println(red("Invalid password. Please check and try again."))
		return false
	}
	...
	fmt.Println("You can now use IPO Pilot for the next 3 months.")
	return true
}
...
func showIntroMsg() {
	...
	Log(cyan("╔════════════════════════════════════════════════════════════════════════════╗"))
	Log(cyan("║") + "                                                                              " + cyan("║"))
	Log(cyan("║") + blue("                          🚀 IPO Pilot 🚀") + "                              " + cyan("║"))
	Log(cyan("║") + "                                                                              " + cyan("║"))
	Log(cyan("║") + green("            Automated IPO Application System") + "                        " + cyan("║"))
	Log(cyan("║") + green("            Monitor & Apply to MeroShare IPOs") + "                      " + cyan("║"))
	Log(cyan("║") + "                                                                              " + cyan("║"))
	Log(cyan("╚════════════════════════════════════════════════════════════════════════════╝"))
	Log("")

	if runtime.GOOS == "windows" {
		Log(cyan("📱 Platform:") + yellow(" Windows Desktop"))
		Log(cyan("✨ Features:") + " Full features available")
		Log(cyan("💡 Tip:") + " You can minimize this window while monitoring")
	} else if runtime.GOOS == "linux" && runtime.GOARCH == "arm64" {
		Log(cyan("📱 Platform:") + yellow(" Android Mobile (Termux)"))
		Log(cyan("✨ Features:") + " Full features available")
		Log(cyan("💡 Tip:") + " Install native APK for better experience")
	} else {
		Log(cyan("📱 Platform:") + yellow(" "+runtime.GOOS+"/"+runtime.GOARCH))
		Log(cyan("✨ Features:") + " All features available")
	}
	Log("")
}
...
func runSelectedProfilesContinuously() {
	...
	if runtime.GOOS == "windows" {
		fmt.Println(green("Windows: Profiles will run continuously in background"))
		fmt.Println(yellow("Tip: You can minimize this window while monitoring"))
	} else if runtime.GOOS == "linux" && runtime.GOARCH == "arm64" {
		fmt.Println(green("Mobile: Profiles will run continuously"))
		fmt.Println(yellow("Tip: Keep Termux active for best results"))
		fmt.Println(yellow("Note: Screen may timeout - monitoring continues"))
	}
...
func showLicenseInfo() {
	...
	fmt.Println(yellow("No renewal needed. You have unlimited access to IPO Pilot."))
	fmt.Println(cyan("For password changes, contact: dallefx (sahmahesh2077@gmail.com)"))
}

func renewLicense() {
	fmt.Println(cyan("\n=== About Your Activation ==="))
	fmt.Println(green("✓ Your account is activated"))
	fmt.Println(green("✓ No renewal needed"))
	fmt.Println(green("✓ Unlimited access"))
	fmt.Println(cyan("For password changes, contact: dallefx (sahmahesh2077@gmail.com)"))
...
func showPlatformInfo() {
	...
	if runtime.GOOS == "windows" {
		...
		fmt.Println("• Full desktop experience")
		fmt.Println("• All features available")
		fmt.Println("• Easy file management")
		fmt.Println("• Background operation support")
		fmt.Println("\nFor mobile users:")
		fmt.Println("• Use Termux app for Android")
		fmt.Println("• Or install native Android APK")
	} else if runtime.GOOS == "linux" && runtime.GOARCH == "arm64" {
		fmt.Println(green("\n✓ Android Mobile Version (Termux)"))
		fmt.Println("• Full mobile compatibility")
		fmt.Println("• All features available")
		fmt.Println("• Optimized for touch interfaces")
		fmt.Println("• Battery-efficient operation")
		fmt.Println("\nFor better experience:")
		fmt.Println("• Install native Android APK")
		fmt.Println("• Modern UI and better performance")
	} else {
		fmt.Println(yellow("\n⚠ Cross-Platform Version"))
		fmt.Printf("• Running on %s/%s\n", runtime.GOOS, runtime.GOARCH)
		fmt.Println("• Full functionality available")
		fmt.Println("• May require platform-specific setup")
	}

	fmt.Println(cyan("\n=== Quick Setup Guide ==="))
	if runtime.GOOS == "windows" {
		fmt.Println("Windows Setup:")
		fmt.Println("1. Extract files to any folder")
		fmt.Println("2. Open Command Prompt in that folder")
		fmt.Println("3. Run: go run ipo_master_customer.go")
		fmt.Println("4. Activate license and create profile")
	} else if runtime.GOOS == "linux" {
		if runtime.GOARCH == "arm64" {
			fmt.Println("Android/Termux Setup:")
			fmt.Println("1. Install Termux from F-Droid")
			fmt.Println("2. Transfer ipo_master_mobile file")
			fmt.Println("3. Run: chmod +x ipo_master_mobile")
			fmt.Println("4. Run: ./ipo_master_mobile")
		} else {
			fmt.Println("Linux Setup:")
			fmt.Println("1. Install Go: sudo apt install golang")
			fmt.Println("2. Run: go run ipo_master_customer.go")
		}
	}
}
...
mainMenu:
	for {
		fmt.Println(cyan("\n=== IPO Pilot - Customer Edition ==="))

		// Platform-specific header
		if runtime.GOOS == "windows" {
			fmt.Println(yellow("Windows Desktop Version - Full Features Available"))
		} else if runtime.GOOS == "linux" && runtime.GOARCH == "arm64" {
			fmt.Println(yellow("Mobile Version (Android/Termux) - Full Features Available"))
		} else {
			fmt.Println(yellow("Cross-Platform Version - " + runtime.GOOS + "/" + runtime.GOARCH))
		}
		...
		fmt.Println("6. Platform information")
		fmt.Println("7. Exit")

		if runtime.GOOS == "windows" {
			fmt.Print("Choose an option (1-7): ")
		} else {
			fmt.Print("Choose an option (1-7) [Mobile users: Use numbers only]: ")
		}
