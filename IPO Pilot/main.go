...
func checkLicense() bool {
	...
	if _, err := os.Stat(licenseFile); os.IsNotExist(err) {
		return false // No license, need to activate
	}

	// Read and decrypt license
	encryptedLicense, err := os.ReadFile(licenseFile)
	if err != nil {
		LogLevel("ERROR", "Failed to read license file")
		return false
	}

	// Simple decryption (in production, use proper encryption)
	licenseData := string(encryptedLicense)
	
	var license License
	err = json.Unmarshal([]byte(licenseData), &license)
	if err != nil {
		LogLevel("ERROR", "Invalid license format")
		return false
	}
	...
	if time.Now().After(license.ExpiryDate) {
		LogLevel("ERROR", "License has expired. Please renew your license.")
		return false
	}

	LogLevel("SUCCESS", fmt.Sprintf("License valid until %s", license.ExpiryDate.Format("2006-01-02")))
	return true
}
...
func activateLicense() bool {
	...
	license := License{
		Key:         key,
		MachineID:   machineID,
		ExpiryDate:  time.Now().AddDate(0, 5, 0), // 5 months
	...
	if err != nil {
		LogLevel("ERROR", "Failed to save license")
		return false
	}
...
func validateLicenseKey(key string) bool {
	...
	// In production, this would validate against a server or use proper cryptography
...
func generateLicenseKey() string {
	...
	// In production, this would be done by a license server
...
func LogLevel(level, msg string) {
	...
	file, err := os.OpenFile("IPO~Master-Log.txt", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
